...
{
    public function SupervisorStockManagementPage(Request $request)
    {
    ...
    public function SupervisorStockUpdateProduct(Request $request, $id)
    {
        $request->validate([
        ...
        return redirect()->route('supervisor.stock.management.page')->with('success', 'Product updated successfully.');
    }

    public function SupervisorStockArchiveProduct($id)
    {
        if (!Auth::guard('employees')->check() || Auth::guard('employees')->user()->position_id != 4) {
            return redirect()->route('login.page')->with('error', 'You must be logged in as an supervisor.');
        }
        ...
        return redirect()->route('supervisor.stock.management.page')->with('success', 'Product archived successfully.');
    }

    public function SupervisorStockPurchaseOrderPage()
    {
        if (!Auth::guard('employees')->check() || Auth::guard('employees')->user()->position_id != 4) {
            return redirect()->route('login.page')->with('error', 'You must be logged in as an supervisor.');
        }
        ...
        return view('supervisor.purchase_order', compact('lowStockProducts', 'poNumber', 'user', 'supervisors', 'suppliers'));
    }


    public function SupervisorStockSubmitPO(Request $request)
    {
        $request->validate([
        ...
        return redirect()->route('supervisor.stock.management.page')->with('success', 'Downloaded successfully!');
    }

    public function SupervisorViewPO($po_number)
    {
        if (!Auth::guard('employees')->check() || Auth::guard('employees')->user()->position_id != 4) {
            return redirect()->route('login.page')->with('error', 'You must be logged in as an supervisor.');
        }
        ...
        if ($purchaseOrderItems->isEmpty()) {
            return redirect()->route('supervisor.view.po.history')->with('error', 'Purchase Order not found.');
        }
        ...
        return view('supervisor.history.view_po_history', compact('purchaseOrderItems', 'po_number', 'supplier'));
    }
}
