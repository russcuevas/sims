...
{
    public function SupervisorDeliveryPage(Request $request)
    {
        ...
        return view('supervisor.delivery_management', compact(
            'role',
            'user',
            'lowFinishedProducts',
            'products',
    ...
    public function SupervisorDeliverySubmitBatch(Request $request)
    {
        if (!Auth::guard('employees')->check() || Auth::guard('employees')->user()->position_id != 4) {
            return redirect()->route('login.page')->with('error', 'You must be logged in as an supervisor to access the dashboard.');
        }
        ...
        return redirect()->route('supervisor.delivery.management.page')->with('success', 'Batch products added successfully!');
    }



    public function SupervisorDeliveryAddStore(Request $request)
    {
        $request->validate([
        ...
        return redirect()->route('supervisor.delivery.management.page')->with('success', 'Store added successfully!');
    }

    public function supervisorAddCar(Request $request)
    {
        $request->validate([
        ...
        return redirect()->route('supervisor.delivery.management.page')->with('success', 'Car added successfully!');
    }

    public function SupervisorDeliveryRemoveProduct($id)
    {
        $product = BatchFetchFinishProducts::find($id);

        if ($product) {
            $product->delete();
            return redirect()->route('supervisor.delivery.management.page')->with('success', 'Product removed successfully!');
        } else {
            return redirect()->route('supervisor.delivery.management.page')->with('error', 'Product not found.');
        }
    }

    public function SupervisorDeliveryAdd(Request $request)
    {
        ...
        return redirect()->route('supervisor.delivery.management.page')
            ->with('success', 'Delivery added successfully!');
    }

    public function SupervisorViewDeliveryOrder($transact_id)
    {
        if (!Auth::guard('employees')->check() || Auth::guard('employees')->user()->position_id != 4) {
            return redirect()->route('login.page')->with('error', 'You must be logged in as an supervisor to access the dashboard.');
        }
        ...
        return view('supervisor.delivery.delivery_order', compact('delivery', 'first'));
    }

    public function SupervisorArchiveDeliveryOrder($transact_id)
    {
        if (!Auth::guard('employees')->check() || Auth::guard('employees')->user()->position_id != 4) {
            return redirect()->route('login.page')->with('error', 'You must be logged in as an supervisor to access the dashboard.');
        }
    ...
    public function SupervisorValidatePinDelivery(Request $request)
    {
        if (!Auth::guard('employees')->check() || Auth::guard('employees')->user()->position_id != 4) {
            return redirect()->route('login.page')->with('error', 'You must be logged in as an supervisor to access the dashboard.');
        }
