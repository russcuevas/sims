...
{
    public function AdminEmailPage()
    {
        ...
        return view('admin.email', compact('role', 'user', 'emails', 'suppliers'));
    }
    ...
    public function EmailBulkDelete(Request $request)
    {
        $ids = explode(',', $request->email_ids);
        Email::whereIn('id', $ids)->update(['is_archived' => 1]);
        return redirect()->back()->with('success', 'Selected emails deleted successfully.');
    }

    public function AdminEmailTrash()
    {
        if (!Auth::guard('employees')->check() || Auth::guard('employees')->user()->position_id != 1) {
            return redirect()->route('login.page')->with('error', 'You must be logged in as an admin to access the dashboard.');
        }
        ...
        return view('admin.email_trash', compact('role', 'user', 'emails'));
    }

    public function EmailBulkRestore(Request $request)
    {
        $ids = explode(',', $request->email_ids);
    ...
    public function EmailBulkDeletePermanent(Request $request)
    {
        $ids = explode(',', $request->email_ids);
