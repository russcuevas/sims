...
{
    public function ManagerStockInPage(Request $request)
    {
    ...
    public function ManagerAddProduct(Request $request)
    {
        ...
        return redirect()->route('manager.stock.in.page')->with('success', 'Product added successfully!');
    }

    public function ManagerAddSupplier(Request $request)
    {
        if (!Auth::guard('employees')->check() || Auth::guard('employees')->user()->position_id != 2) {
            return redirect()->route('login.page')->with('error', 'You must be logged in as an manager.');
        }
        ...
        return redirect()->route('manager.stock.in.page')->with('success', 'Supplier added successfully!');
    }

    public function ManagerAddBatchProductDetails(Request $request)
    {
        if (!Auth::guard('employees')->check() || Auth::guard('employees')->user()->position_id != 2) {
            return redirect()->route('login.page')->with('error', 'You must be logged in as an manager to access the dashboard.');
        }
    ...
    public function ManagerupdateQuantity(Request $request, $id)
    {
        $request->validate([
    ...
    public function ManagerRemoveBatchProduct($id)
    {
        if (!Auth::guard('employees')->check() || Auth::guard('employees')->user()->position_id != 2) {
            return redirect()->route('login.page')->with('error', 'You must be logged in as an manager to access the dashboard.');
        }
        ...
        if ($deleted) {
            return redirect()->route('manager.stock.in.page')->with('success', 'Batch product removed successfully.');
        } else {
            return redirect()->route('manager.stock.in.page')->with('error', 'Failed to remove batch product.');
        }
    }


    public function ManagerRawStocksRequest(Request $request)
    {
        if (!Auth::guard('employees')->check() || Auth::guard('employees')->user()->position_id != 2) {
            return redirect()->route('login.page')->with('error', 'You must be logged in as an manager to access the dashboard.');
        }
    ...
    public function ManagerArchiveRawStock($transactId)
    {
        if (!Auth::guard('employees')->check() || Auth::guard('employees')->user()->position_id != 2) {
            return redirect()->route('login.page')->with('error', 'You must be logged in as an manager to access the dashboard.');
        }
